...
- **CSP（约束 #8）**：生产 `default-src 'self'`；开发模式放行 `script-src` / `style-src 'self' 'unsafe-inline'`（react-refresh 内联脚本与 vite client 内联样式，2026-08-07 用户裁决）+ `connect-src ws://localhost:*`（HMR）；不放开任何远程加载 / `unsafe-eval`
...
// 开发模式：react-refresh 内联脚本 / vite client 内联样式（unsafe-inline）+ HMR WebSocket（用户 2026-08-07 裁决）
