...
- [x] **Step 1: 失败测试**（fixture 账本含 Breakfast: Expenses:Food + Assets:Bank 两行；`listEntries({ account: 'Expenses:Food' })` 只返回 Food 行，total=该账户行数；`account` 与 `dateFrom` 叠加生效；未知账户返回空）
- [x] **Step 2: 跑失败** `npx vitest run src/main/index-builder.test.ts` → FAIL
- [x] **Step 3: 实现**：入参校验（`account` 为非空字符串，长度上限 200）；SQL 在现有 WHERE 链上加 `eq(postings.account, account)`（drizzle 参数化，无注入面）；`total` 计数同条件
- [x] **Step 4: 跑通过** → PASS；`npm run typecheck`
- [x] **Step 5: Commit** `feat(cap-f): listEntries 账户精确过滤参数`
...
- [x] **Step 1: 筛选区** —— `TreeSelect`（数据源 `accountOptions` 按五大类分组，`treeNodeFilterProp="label"`，allowClear）+「查询」按钮；选择后 `loadEntries({ account, order: 'desc', limit: 20, offset })`
- [x] **Step 2: 明细表** —— `Table` 列：日期 / 交易对象 / 说明 / 账户（中文映射 + Tooltip 原名）/ 金额（`formatAmount(row.amount)`，`.num` 右对齐，`amount === null` 显示 `—`）/ 币种；`order: 'desc'` 默认倒序；分页同 EntriesView 模式（`total` + `offset`）；本地状态 `useState` 保存当前查询参数，切换账户重置页码
- [x] **Step 3: 空态与加载** —— 未选账户：`Empty description="选择账户后查看其明细分录"`；选中无数据：`Empty description="该账户暂无分录"`；加载态 Table `loading`
- [x] **Step 4: 验证**：typecheck + unit；`npm run dev`（测试账本）手动核对；`npm run test:e2e -- --grep "ledger|index"` 回归
- [x] **Step 5: Commit** `feat(cap-f): 对账页明细账 Tab 接真数据（账户过滤 + 金额列）`

### Task 3: E2E 与收尾

- [x] **Step 1: e2e**（`e2e/reconcile.spec.ts` 新增最小用例：进对账页 → Tab② TreeSelect 选 Expenses:Food → 表格出现行且账户列含 Food；fixture 复用 `createFixtureCopy` 模式）
