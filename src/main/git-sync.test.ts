...
// ==================== 本地 git smart-HTTP 测试服务器（纯 JS，零外部 git 二进制） ====================
// isomorphic-git 1.41.3 仅支持 http/https 传输（file:// 本地传输已在 1.x 移除），
// 因此单测用进程内 HTTP 服务器暴露本地裸仓，实现最小可行 smart HTTP 协议：
...
/** 本地裸仓（E2E/单测通用 origin；零外部 git 依赖） */
 ...
 * clone 显式 ref: main（空裸仓 clone 出的本地仓库 HEAD 停在 master——clone 内部 init 的默认分支），
 * commit 同样显式 ref: main，保证 push({ ref: 'main' }) 能解析到本地分支。
 */
async function seedRemoteCommit(remoteUrl: string): Promise<void> {
  ...
  try {
    ...
    await git.commit({ fs, dir: workDir, message: 'seed: 远端提交', author: GIT_AUTHOR, ref: SYNC_BRANCH })
...
describe('GitSync（M6）', () => {
  ...
  it('analyzeMerge：远端领先（同祖先）→ fast-forward + theirsContent', async () => {
    ...
    await sync.addRemote(remoteUrl)
    await sync.push()
    await seedRemoteCommit(remoteUrl) // 远端新增一笔
    await sync.fetch()
    const status = await sync.analyzeMerge()
  ...
  it('analyzeMerge：两端改不同位置 → clean-merge（diff3 自动合并）', async () => {
    ...
    await sync.addRemote(remoteUrl)
    await sync.push()
    await seedRemoteCommit(remoteUrl) // 远端在 EOF 追加一笔
    await sync.fetch()
  ...
  it('listServerRefs：空仓 → []；非空 → 有 ref', async () => {
    ...
    await seedRemoteCommit(remoteUrl)
